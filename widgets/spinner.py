...
class QtSpinner(QWidget):
    def __init__(self, parent=None, center_on_parent=True, disable_parent_when_spinning=False,
        ...
        # WAS IN initialize()
        self._color = QColor(QtCore.Qt.green)
    ...
    def update_timer(self):
        self._timer.setInterval(1000 // int(self._numberOfLines * self._revolutionsPerSecond))

    def update_position(self):
        if self.parentWidget() and self._center_on_parent:
            parent_rect = QtCore.QRect(self.parentWidget().mapToGlobal(QtCore.QPoint(0, 0)), self.parentWidget().size())
            self.move(
                QStyle.alignedRect(QtCore.Qt.LeftToRight, QtCore.Qt.AlignCenter, self.size(), parent_rect).topLeft())
    ...
    @staticmethod
    def current_line_color(count_distance, total_nr_of_lines, trail_fade_perc, min_opacity, colorinput):
