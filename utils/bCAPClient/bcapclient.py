...
from utils.bCAPClient.orinexception import *
from threading import Lock
from utils.bCAPClient.variant import VarType
...
class BCAPClient:
    ...
    def _serialize_args(self, args, first=False):
        ...
        if len(packet_data) > 0:
            return struct.pack(_format, *packet_data)
        else:
            return b''
    ...
    def _deserialize_arg(self, buf):
        ...
        _format = "<HI%ds" % (len(buf) - 6)
        (vt, len_arg, buf) = struct.unpack(_format, buf)
        ...
        if (vt & VarType.VT_ARRAY) != 0:
            ...
            if vt == VarType.VT_VARIANT:
                ...
                _format = "<%ds%ds" % (len_arg, len(buf) - len_arg)
                (retval, buf) = struct.unpack(_format, buf)
                ...
                if vt == VarType.VT_BSTR:
                    ...
                    for i in range(len_arg):
                        _format = "<I%ds" % (len(buf) - 4)
                        (len_str, buf) = struct.unpack(_format, buf)
                        _format = "<%ds%ds" % (len_str, len(buf) - len_str)
                        (ret_tmp, buf) = struct.unpack(_format, buf)
                        retval.append(ret_tmp.decode("utf-16le"))

                else:
                    _format = "<%s%ds" % (fmt * len_arg, len(buf) - (len_val * len_arg))
                    unpacked_arg = struct.unpack(_format, buf)
            ...
            if vt in [VarType.VT_EMPTY, VarType.VT_NULL]:
                ...
                if vt == VarType.VT_BSTR:
                    ...
                    _format = "<%s%ds" % (fmt, (len(buf) - len_val))
                    (retval, buf) = struct.unpack(_format, buf)
